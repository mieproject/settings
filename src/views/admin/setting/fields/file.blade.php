...
<div class="form-group {{ $errors->has($field['name']) ? ' has-error' : '' }}">
    ...
    @if(isset($field['data']) and $field['data'] == 'json' and is_array(setting($field['name'])))
        @if(setting($field['name']) != null || isset($field['value']))
            @foreach(setting($field['name']) as $img)
                ...
                <script>
                    ...
                    $(document).ready(function () {
                        $("img")
                            ...
                            .on('error', function() {
                                var sarr = $('input[name="' + $('div.slider').data('name') + '"]').val().split(",");
                    ...
                    $('.{{$field['name']}}_remove').click(function () {
                        sarr = $('input[name="' + $('div.slider').data('name') + '"]').val().split(",");
        ...
        @endif
            ...
            <input type="hidden"
                   name="{{ $field['name'] }}"
                   value="{{ old($field['name'],  implode(',',setting($field['name'])) ?? @$field['value']) }}"
